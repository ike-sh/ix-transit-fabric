...
每条转发规则包含规则 ID、备注、启用状态、客户端入口端口、虚拟网中转端口、落地机地址、落地业务端口和转发协议。NAT IX 机器新增落地规则并刷新接入码；公网入口机重新导入接入码时，会逐条显示远端规则，并按 `rule_id` 更新本地规则。公网入口机必须为每条启用规则指定独立客户端入口端口，不允许端口复用；停用规则不转发。远端已删除的规则默认提示停用本地对应规则，不会自动删除。删除规则后需要刷新接入码并重新导入。
